...
rule run_semibin2:
    ...
    resources:
        mem_mb = config.resources.big.mem,
        time = config.resources.big.time,
        partition=config.resources.gpu.partition, # to send it to gpu partition
        slurm=config.resources.gpu.slurm # command for slurm
    params:
    ...
    shell:
        """
        SemiBin2 multi_easy_bin -i {input.catalogue}  -b {input.bams} -o {params.outdir} -s {params.separator} --minfasta-kbs {params.minfasta} -r {params.databases}
        touch {output.outtouch}
        """
